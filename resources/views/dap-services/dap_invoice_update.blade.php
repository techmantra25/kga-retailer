  ...
  <table style="border: 1px solid black; width: 700px; margin: auto; font-family: Arial, sans-serif; border-collapse: collapse;">
    ...
    <tr>
      <td colspan="9" style="padding: 10px;">
        <p style="margin: 0;"><b>Branch Name:</b> {{ $data->branch?$data->branch->name:"" }} </p>
        <p style="margin: 10px 0;"><b>Address:</b> {{ $data->branch?$data->branch->address:"" }}</p>
        <p style="margin: 10px 0;"><b>Phone:</b> {{ $data->branch?$data->branch->phone:"" }}</p>
      </td>
    </tr>
    <tr>
    ...
    <tr>
      <td colspan="4" style="border: 1px solid black; padding: 10px; vertical-align: top;">
        ...
        <p><b>Name :</b> {{ $data->customer_name?$data->customer_name:"" }}</p>
        <p><b>Address :</b> {{ $data->address?$data->address:"" }}</p>
        <p><b>Contact No :</b> {{ $data->alternate_no?$data->alternate_no:"" }}</p>
      ...
      <td colspan="5" style="border: 1px solid black; padding: 10px; vertical-align: top;">
        <p><b>Invoice No :</b> {{ $data->paymentData?$data->paymentData->invoice_id:"" }}</p>
        <p><b>Date :</b> {{$Todate}}</p>
        <p><b>Call No :</b> {{ $data->unique_id?$data->unique_id:"" }}</p>
        <p><b>Call Date :</b> {{ $data->entry_date?$data->entry_date:"" }}</p>
        <p><b>Product :</b>{{ $data->item?$data->item:"" }} </p>
        <p><b>Product Sl. No :</b> {{ $data->serial?$data->serial:"" }}</p>
      </td>
    </tr>
    ...
    @php
        $totalQty += $item->qty;
        $rate_amount = $item->final_amount ? $item->final_amount : 0;
        $total_rate_amount += $rate_amount;
        $final_amount = $item->final_amount ? $item->final_amount : 0;
    ...
    <!-- Services Section -->
     @php
     $service_cgst = ($data->repair_charge *(($gst_value/2)/100));
     $service_sgst = ($data->repair_charge *(($gst_value/2)/100));
     $service_taxable_vale = $data->repair_charge-$service_cgst-$service_sgst;
     $grand_total_amount=$total_final_amount+$data->repair_charge;
     @endphp
    <tr>
    ...
    <tr style="text-align: center;">
      ...
      <td style="border: 1px solid black;padding: 3px;">{{number_format($data->repair_charge,2)}}</td>
    </tr>
    <!-- <tr style="text-align: center; font-weight: bold;">
      <td colspan="" style="border: 1px solid black;text-align: center;padding: 3px;">Total</td>
    ...
    <tr style="font-weight: bold; text-align: left;padding: 3px;">
      ...
      <td colspan="1" style="border: 1px solid black;padding: 3px;border-left: 0;">{{ $data->paymentData?$data->paymentData->payment_id:""}}</td>
      <td colspan="4" style="border: 1px solid black;padding: 8px;border-right:0;text-align:right;">PAYMENT Date :</td>
      <td colspan="3" style="border: 1px solid black;padding: 8px;border-left:0;text-align:right;">{{ $data->paymentData?$data->paymentData->payment_date:"" }}</td>
    </tr>
