...
<section>    
    <ul class="breadcrumb_menu">  
        <li>Chimney Maintenance & Repair Notifications</li>     
        <li>{{ $maintenance->unique_id }}</li>
        <li>Add Spares</li>
    </ul>
    <div class="row">        
        <form id="myForm" action="{{ route('servicepartnerweb.maintenance.save-spare-parts',$getQueryString) }}" method="POST">
        @csrf
            ...
            <div class="col-sm-12">                  
                    ...
                    <div class="table-responsive order-addmore">
                        <table class="table" id="timePriceTable">
                            ...
                            <tbody> 
                                ...
                                @php
                                    $getMaintenanceSpares = getMaintenanceSpares($maintenance_id,$details['product_id']);

                                    $isNew = 0;
                                    $readonlyOld = "";
                                    if(!empty($getMaintenanceSpares)){
                                        $isNew = 1;
                                ...
                                <tr id="tr_{{$key}}" class="tr_pro">
                                    <td class="f-12" style="width: 690px;">
                                        <input type="text" class="form-control" id="product{{$key}}" onkeyup="getProducts(this.value,{{$key}});" placeholder="Search spare ... " name="details[{{$key}}][product]" value="{{ old('details.'.$key.'.product') }}" {{$readonlyOld}}>
                                ...
                                @if(!empty($data))
                                    ...
                                    <tr id="tr_{{$j}}" class="tr_pro">
                                        <td class="f-12" style="width: 690px;">
                                            <input type="text" class="form-control" id="product{{$j}}" onkeyup="getProducts(this.value,{{$j}});" placeholder="Search spare ... " name="details[{{$j}}][product]" value="{{ getSingleAttributeTable('products','id',$item['product_id'],'title') }}" readonly>
                                ...
                                <tr id="tr_1" class="tr_pro">
                                    <td class="f-12" style="width: 690px;">
                                        <input type="text" class="form-control" id="product1" onkeyup="getProducts(this.value,1);" placeholder="Search spare ... " name="details[1][product]" >
                ...
                <div class="card shadow-sm">
                    ...
                    <div class="card-body text-end">
                        <a href="{{ route('servicepartnerweb.maintenance.clear-spares',[Crypt::encrypt($maintenance_id),$getQueryString]) }}" onclick="return confirm('Are you sure want to clear all spare?');" class="btn btn-sm btn-warning">Clear Spare Requisition</a>
                    </div>
                    @endif
                    
                    <div class="card-body text-end">
                        
                        <a href="{{ route('servicepartnerweb.maintenance.list') }}?{{$getQueryString}}" class="btn btn-sm btn-danger">Back</a>
...
<script>
    ...
    $(document).on('click','.addNewTime',function(){
        ...
        <tr id="tr_`+i+`" class="tr_pro">
            <td class="f-12" style="width: 690px;">
                <input type="text" class="form-control" id="product`+i+`" placeholder="Search spare ... " onkeyup="getProducts(this.value,`+i+`);" name="details[`+i+`][product]">
