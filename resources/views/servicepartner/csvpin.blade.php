...
<section>   
    ...
    <div class="row">
                        ...
                        <div class="col-md-3">
                            ...
                            <div class="form-group">
                                ...
                                <select name="product_type" class="form-control" id="">
									...
									<option value="gieger">Geyser</option>
                                </select>
        ...
        <div>
            ...
            <h6>
                Assigned Pincodes For Gas-stove Goods 
                @if(count($service_partner_pincodes_gas_stove) > 0) <strong>({{count($service_partner_pincodes_gas_stove)}})</strong> @endif
            </h6>
            <ul class="pincodeclass">
                @forelse ($service_partner_pincodes_gas_stove as $pincode)
                    <li>{{$pincode->number}}</li>
                @empty
                    <li> - No Pincodes Found -</li>
                @endforelse
            ...
            <h6>
                Assigned Pincodes For Ac Goods 
                @if(count($service_partner_pincodes_ac) > 0) <strong>({{count($service_partner_pincodes_ac)}})</strong> @endif
            </h6>
            <ul class="pincodeclass">
                @forelse ($service_partner_pincodes_ac as $pincode)
                    <li>{{$pincode->number}}</li>
                @empty
                    <li> - No Pincodes Found -</li>
                @endforelse
			...
			<h6>
                Assigned Pincodes For Gieger Goods 
                @if(count($service_partner_pincodes_gieger) > 0) <strong>({{count($service_partner_pincodes_gieger)}})</strong> @endif
            </h6>
            <ul class="pincodeclass">
                @forelse ($service_partner_pincodes_gieger as $pincode)
                    <li>{{$pincode->number}}</li>
                @empty
                    <li> - No Pincodes Found -</li>
                @endforelse
